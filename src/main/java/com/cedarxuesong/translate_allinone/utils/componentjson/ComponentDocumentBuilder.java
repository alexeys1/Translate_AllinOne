...
public final class ComponentDocumentBuilder {
    ...
    public ComponentTranslationDocument build(Text component, ComponentTranslationRoute route) {
        return build(ComponentJsonCodec.encode(component), ComponentTranslationPolicy.forRoute(route));
    }

    public ComponentTranslationDocument build(Text component, ComponentTranslationPolicy policy) {
        return build(ComponentJsonCodec.encode(component), policy);
    }
    ...
    private void collectComponent(
        ...
        if (text != null) {
            if (!text.isJsonPrimitive() || !text.getAsJsonPrimitive().isString()) {
                throw new ComponentJsonException(ComponentJsonException.Kind.DOCUMENT, "Literal Text text is not a string.");
            }
