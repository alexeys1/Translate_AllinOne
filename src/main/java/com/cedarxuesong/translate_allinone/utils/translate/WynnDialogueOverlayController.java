...
import com.cedarxuesong.translate_allinone.mixin.mixinWynnDialogue.WynnDialogueInGameHudAccessor;
import com.cedarxuesong.translate_allinone.mixin.mixinWynnDialogue.WynnDialogueTextRendererAccessor;
...
public final class WynnDialogueOverlayController {
    ...
    private void acceptPresentation(WynnDialoguePresentation presentation, boolean refreshOverlay) {
        ...
        boolean hudProgressCompatible = hudMode
                && activeTemplate.observedNonce() != 0L
                && matchesActiveNonce
                && hudSourcesCanAdvance(activeTemplate.template(), presentation);
        boolean compatibleWithActive = sourcesCompatible(activeTemplate.template(), presentation)
                || hudProgressCompatible;
    ...
    private Component renderForSetter() {
        if (activeTemplate == null) {
            return Component.empty();
        }
        Component rendered = activeTemplate.template().originalText();
        if (!useHud.getAsBoolean()
                && (showTranslated.getAsBoolean() || hasPendingPresentation(currentPresentation))) {
    ...
    private static boolean hasPendingPresentation(WynnDialoguePresentation presentation) {
        return presentation != null
                && (presentation.dialoguePending()
                || presentation.optionsPending()
                || presentation.options().stream().anyMatch(WynnDialoguePresentation.OptionPresentation::pending));
    }
    ...
    private void replaceActiveOverlay() {
        ...
        Component current = overlayAccess.currentText();
        ...
        Component replacement = renderForSetter();
    ...
    private void restoreOriginalOverlay() {
        ...
        Component original = activeTemplate.template().originalText();
    ...
    private static int measureText(String value, Style style) {
        ...
        WynnDialogueTextRendererAccessor accessor = (WynnDialogueTextRendererAccessor) client.font;
    ...
    private static boolean supportsText(String value, Style style) {
        ...
        return WynnDialogueInlinePresenter.supportsText(client.font, value, style);
    }
    ...
    private static boolean sourcesCompatible(
        ...
        if (!coreSourcesCompatible(template, presentation)) {
            return false;
        }
    ...
    private static boolean coreSourcesCompatible(
            WynnDialogueTextTemplate template,
            WynnDialoguePresentation presentation
    ) {
    ...
    private static boolean hudSourcesCanAdvance(
            WynnDialogueTextTemplate template,
            WynnDialoguePresentation presentation
    ) {
        if (template == null || presentation == null) {
        ...
        if (!sourceMatchesOrAdvances(template.dialogue(), presentation.originalDialogue())) {
            return false;
        }
        List<String> templateOptions = template.choices().stream()
                .map(WynnDialogueTextTemplate.ChoiceSlot::readableText)
        ...
        List<String> presentationOptions = presentation.options().stream()
                ...
                .filter(value -> !value.isBlank())
                .toList();
        if (templateOptions.isEmpty() || presentationOptions.size() < templateOptions.size()) {
            return false;
        }
        boolean[] matched = new boolean[presentationOptions.size()];
        for (String templateOption : templateOptions) {
            boolean found = false;
            for (int index = 0; index < presentationOptions.size(); index++) {
                if (!matched[index] && sourceMatchesOrAdvances(templateOption, presentationOptions.get(index))) {
                    matched[index] = true;
                    found = true;
                    break;
                }
    ...
    private static boolean sourceMatchesOrAdvances(String currentSource, String presentedSource) {
        String current = normalizedSource(currentSource);
        String presented = normalizedSource(presentedSource);
        if (current.equals(presented) || removeOneLeadingDuplicateWord(current).equals(presented)) {
            return true;
        }
        return !current.isBlank()
                && presented.length() >= current.length()
                && (presented.startsWith(current) || presented.endsWith(current));
    }
    ...
    public interface OverlayAccess {
        ...
        void replaceText(Component text);
    ...
    public interface OverlayMessageObserver {
        WynnDialoguePresentation observe(Component original);
    }
    ...
    private static final class MinecraftOverlayAccess implements OverlayAccess {
        ...
        public Component currentText() {
            ...
            return accessor == null ? Component.empty() : accessor.translate_allinone$getOverlayMessage();
        }

        @Override
        public void replaceText(Component text) {
            WynnDialogueInGameHudAccessor accessor = accessor();
            if (accessor != null) {
