...
public final class WynnDialogueInlinePresenter {
    ...
    public WynnDialogueInlinePresenter(TextRenderer textRenderer) {
        this((value, style) -> textRenderer == null
                ? 0
                : textRenderer.getWidth(Text.literal(value).setStyle(style)),
    ...
    public Text render(WynnDialogueTextTemplate template, WynnDialoguePresentation presentation) {
    ...
    private WynnDialogueInlineRenderResult rebuild(
        ...
        for (WynnDialogueTextTemplate.TemplateToken token : template.tokens()) {
            ...
            if (action.type() == SlotActionType.REPLACE) {
                ...
                if (inserted.add(key)) {
                    ...
                    if (!action.text().isEmpty()) {
                        rebuilt.append(Text.literal(action.text()).setStyle(token.style()));
                        translated = true;
                    }
                    if (!compensation.text().isEmpty()) {
                        rebuilt.append(Text.literal(compensation.text()).setStyle(compensation.style()));
                    }
                }
                continue;
            }
            ...
            if (!maskedToken.text().isEmpty()) {
                rebuilt.append(Text.literal(maskedToken.text()).setStyle(maskedToken.style()));
            }
            masked = true;
        }
    ...
    static boolean supportsText(TextRenderer textRenderer, String value, Style style) {
